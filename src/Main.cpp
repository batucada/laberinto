...
int main()
{
	...
	playerSprite.setScale((float)160.f / playerSprite.getTexture()->getSize().x, (float)40.f / playerSprite.getTexture()->getSize().y);
	sf::IntRect pos_frame_perseo(0, 0, (float)playerSprite.getTexture()->getSize().x / 4.f, (float)playerSprite.getTexture()->getSize().y);
	playerSprite.setTextureRect(pos_frame_perseo);
	...
	fondoTextura.loadFromFile("fondo.png");
	sf::Sprite fondoSprite;
	fondoSprite.setTexture(fondoTextura);
	fondoSprite.setScale((float)600 / fondoTextura.getSize().x, (float)600 / fondoTextura.getSize().y);

	//me parece que inicializa las dimensiones del juego y los rectangulos
	int gameMap[15 * 15];
	...
	for (int i = 0; i < 15; i++)
	{
		for (int j = 0; j < 15; j++)
		{
			displayRects[i + j * 15].setPosition(i * 40.f, j * 40.f);	  //CREO que es la posicion de los cuadrados
			displayRects[i + j * 15].setSize(sf::Vector2f(40.f, 40.f));	  //tamaño de los cuadrados del mapa
			displayRects[i + j * 15].setOutlineThickness(1.f);			  //borde de los cuadrados del mapa
			displayRects[i + j * 15].setOutlineColor(sf::Color(0, 0, 0)); //color de los bordes de los cuadrados

			//Crea los bloques random donde no se encuentre el jugador ni el enemigo
			...
			if (!(i == opponent.x && j == opponent.y) && !(i == player.x && j == player.y))
			{
				...
				{
					gameMap[i + j * 15] = 1;
					displayRects[i + j * 15].setFillColor(sf::Color(0, 0, 0)); // black tiles
				}
			}
		}
	}
	...
	while (window.isOpen())
	{
		...
		{
			if (event.type == sf::Event::Closed) //cierra el programa
			{
				window.close();
			}
			else if (event.type == sf::Event::KeyPressed)
			{
				switch (event.key.code) //Movimientos del personaje
				{
					case sf::Keyboard::Up:
						if (gameMap[player.x + (player.y - 1) * 15] != 1)
						{
							sf::IntRect pos_frame_perseo(35, 0, (float)playerSprite.getTexture()->getSize().x / 4.f, (float)playerSprite.getTexture()->getSize().y);
							playerSprite.setTextureRect(pos_frame_perseo);
							player.y -= 1;
						}
					...
					case sf::Keyboard::Right:
						...
						{
							sf::IntRect pos_frame_perseo(105, 0, (float)playerSprite.getTexture()->getSize().x / 4.f, (float)playerSprite.getTexture()->getSize().y);
							playerSprite.setTextureRect(pos_frame_perseo);
							player.x += 1;
						}
						break;
					case sf::Keyboard::Left:
						if (gameMap[(player.x - 1) + player.y * 15] != 1)
						{
							sf::IntRect pos_frame_perseo(70, 0, (float)playerSprite.getTexture()->getSize().x / 4.f, (float)playerSprite.getTexture()->getSize().y);
							playerSprite.setTextureRect(pos_frame_perseo);
							player.x -= 1;
						}
						break;
